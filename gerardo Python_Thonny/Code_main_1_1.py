...
def buscar_y_atacar():
    global _fase_busqueda, _tiempo_ultimo_cambio, _barridos_sin_rival, _rival_perdido_count
    ...
    if rival_detectado():
        ...
        if ahora - _tiempo_ultimo_cambio > INTERVALO_CAMBIO_GIRO:
            ...
            if _barridos_sin_rival % 3 == 0:
                _fase_busqueda = (_fase_busqueda + 1) % 3
            _tiempo_ultimo_cambio = ahora

        if _fase_busqueda == 0:
            arrancar_motores(1.0, 1.0)
            duracion = 0.25
        elif _fase_busqueda == 1:
            arrancar_motores(-1.0, -1.0)
            duracion = 0.25
        else:
            arrancar_motores(1.0, 1.0)
...
def inicio_ronda(ronda):
    if ronda == 1:
        ...
        arrancar_motores(-1.0, -1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.75:
            if leer_borde_mejorado() is not None: break
