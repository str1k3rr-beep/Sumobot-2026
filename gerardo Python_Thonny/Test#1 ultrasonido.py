...
INTERVALO_CAMBIO_GIRO = 0.4  # segundos entre microajustes de búsqueda


def buscar_y_atacar():
    """
    Si el ultrasonido detecta al rival → ataque directo.
    Si no → gira en micropasos buscándolo hasta encontrarlo
    o hasta que leer_borde() lo interrumpa.
    El time_sleep es tan pequeño que da la ilusión de visión continua.
    """
    global _direccion_giro_busqueda, _tiempo_ultimo_cambio
    ...
    if rival_detectado():
        ...
        ib.motor_1.throttle = -1.0
        ib.motor_2.throttle = 1.0
        sleep(0.02)  # Avanza en micropasos para poder interrumpir con borde
    else:
        # ---- SIN RIVAL: girar buscando ----
        ib.pixel = (255, 165, 0)  # Naranja = modo búsqueda

        ahora = time.monotonic()
        if ahora - _tiempo_ultimo_cambio > INTERVALO_CAMBIO_GIRO:
            # Cambio de dirección aleatorio para no ser predecible
            _direccion_giro_busqueda = random.choice([-1, 1])
            _tiempo_ultimo_cambio = ahora

        # Microgiro: un motor avanza, el otro retrocede
        ib.motor_1.throttle = -0.8 * _direccion_giro_busqueda
        ib.motor_2.throttle =  0.8 * _direccion_giro_busqueda
        sleep(0.02)  # Microstep — tan corto que parece continuo
