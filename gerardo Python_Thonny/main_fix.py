...
# ============================================================
# ARRANQUE SUAVE CONTINUO — rampa perfecta sin picos de corriente
# ============================================================
...
def arrancar_motores(m1, m2, duracion=0.05):
    ...
    while True:
        ...
        if transcurrido >= duracion:
            break
        progreso = transcurrido / duracion  # 0.0 → 1.0 suavemente
...
def calibracion_por_pasos():
    ...
    for _ in range(2):
        ...
        ib.pixel = (0, 0, 0)
        sleep(0.4)
    sleep(0.3)  # Pausa extra antes de arrancar motores
...
def leer_borde_mejorado():
    ...
    d1 = leer()
    sleep(0.002)  # reducido de 0.005 — suficiente para sensores IR
    d2 = leer()
...
def maniobra_escape(direccion):
    ...
    # Retroceso base con arranque continuo + monotonic
    ...
    while time.monotonic() - start < 0.15:
        ...
        pass  # monotonic — no duerme, sigue revisando
...
def inicio_ronda(ronda):
    sleep(0.2)  # Estabilizar después de calibración
    ...
    if ronda == 1:
        ...
        arrancar_motores(-1.0, -1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.35:  # Ajustar para 90° exacto
            if leer_borde_mejorado() is not None: break
        ...
        arrancar_motores(1.0, 1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.7:   # Ajustar para 180° exacto
            if leer_borde_mejorado() is not None: break
...
while ronda <= 3:
    ...
    while True:
        ...
        # Una sola lectura de borde por iteración
