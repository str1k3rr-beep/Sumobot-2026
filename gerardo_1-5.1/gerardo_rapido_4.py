...
def maniobra_escape(direccion):
    ...
    if direccion == "EMPUJE_IZQUIERDA":
        ib.pixel = (0, 255, 255)
        # Rival empuja desde la izquierda → lado derecho del robot está en el borde
        # Paso 1: avanzar hacia adelante para salir del blanco (romper el empuje)
        ...
        # Paso 2: girar sobre el eje hacia la izquierda para encarar al rival
        arrancar_motores(1.0, 1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.25:
            if boton_presionado(): return True
            if leer_borde_mejorado() is not None: break
        # Paso 3: contraataque directo
        arrancar_motores(-1.0, 1.0)
        start = time.monotonic()
    ...
    if direccion == "EMPUJE_DERECHA":
        ib.pixel = (0, 255, 255)
        # Rival empuja desde la derecha → lado izquierdo del robot está en el borde
        # Paso 1: avanzar hacia adelante para salir del blanco (romper el empuje)
        arrancar_motores(-1.0, 1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.2:
            if boton_presionado(): return True
            if leer_borde_mejorado() is None: break
        # Paso 2: girar sobre el eje hacia la derecha para encarar al rival
        arrancar_motores(-1.0, 1.0)
        start = time.monotonic()
        while time.monotonic() - start < 0.25:
            if boton_presionado(): return True
