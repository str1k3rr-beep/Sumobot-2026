...
def mover(m1, m2, duracion, salir_si_sin_borde=False, salir_si_borde=False):
    """Arranca motores y espera duracion segundos.
    Aborta si se presiona el botón (retorna True).
    salir_si_sin_borde: corta el loop cuando NO hay borde (break).
    salir_si_borde: corta el loop cuando SÍ hay borde (break).
    Retorna False si terminó sin abortar."""
    arrancar_motores(m1, m2)
    start = time.monotonic()
    while time.monotonic() - start < duracion:
        if boton_presionado():
            return True
        if salir_si_sin_borde and leer_borde_mejorado() is None:
            break
        if salir_si_borde and leer_borde_mejorado() is not None:
            break
    return False
...
def maniobra_escape(direccion):
    ...
    if direccion == "EMPUJE_IZQUIERDA":
        ib.pixel = (0, 255, 255)
        if mover(-1.0, 1.0, 0.35): return True
        if mover(1.0, 1.0, 0.35): return True
        if mover(-1.0, 1.0, 0.3): return True
        _tiempo_inicio_borde = None
        frenar()
        return False
    ...
    if tiempo_en_borde > 0.4 and not rival_detectado():
        ...
        if direccion == "IZQUIERDA":
            ib.pixel = (0, 255, 255)
            if mover(-1.0, -1.0, 0.3, salir_si_sin_borde=True): return True
            if mover(-1.0, 1.0, 0.25, salir_si_borde=True): return True
            ...
            if mover(-1.0 * lado, -1.0 * lado, 0.35, salir_si_sin_borde=True): return True
            if mover(-1.0, 1.0, 0.2, salir_si_borde=True): return True
    ...
    # Escape normal
    if mover(1.0, -1.0, 0.15, salir_si_borde=True): return True
    frenar()

    if direccion in ("FRENTE", "FRENTE_IZQ"):
        if mover(-1.0, -1.0, 0.45, salir_si_borde=True): return True

    elif direccion == "IZQUIERDA":
        if mover(1.0, 1.0, 0.3, salir_si_borde=True): return True
...
def buscar_y_atacar():
    ...
    if rival_detectado():
        ...
        if _rival_perdido_count < MAX_PERDIDO:
            ib.pixel = (0, 0, 255)
            if mover(-1.0, 1.0, 0.05, salir_si_borde=True): return True
            return False
...
def inicio_ronda(ronda):
    ...
    if ronda == 1:
        ...
        if mover(-1.0, 1.0, 0.3, salir_si_borde=True): pass

    elif ronda == 2:
        ib.pixel = COLORES_RONDA[2]
        if mover(-1.0, -1.0, 0.35, salir_si_borde=True): pass

    elif ronda == 3:
        ib.pixel = COLORES_RONDA[3]
        if mover(-1.0, -1.0, 0.95, salir_si_borde=True): pass
